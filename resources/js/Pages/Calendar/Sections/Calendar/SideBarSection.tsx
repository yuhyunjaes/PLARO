...
import {faClock, faUpRightAndDownLeftFromCenter, faDownLeftAndUpRightToCenter} from "@fortawesome/free-solid-svg-icons";
...
interface SideBarSectionProps {
    ...
    setDescriptionEditingStatus: (editing: boolean) => void;
    aiSourceText: string;
    setAiSourceText: Dispatch<SetStateAction<string>>;
    aiSummary: string;
    aiSummarizing: boolean;
    summarizeEventWithAi: () => Promise<void>;
    applyAiSummaryToDescription: () => void;
...
export default function SideBarSection({
                                           ...
                                           eventDescription,
                                           setEventDescription,
                                           setDescriptionEditingStatus,
                                           aiSourceText,
                                           setAiSourceText,
                                           aiSummary,
                                           aiSummarizing,
                                           summarizeEventWithAi,
                                           applyAiSummaryToDescription,
                                           eventColor,
                                           setEventColor,
                                           eventTitle,
                                           setEventTitle,
    ...
    const [fullSizeSideBarToggle, setFullSizeSideBarToggle] = useState<boolean>(false);
    ...
    const editingParticipantLabels = editingParticipantIds.map((userId) => {
        const participant = eventParticipants.find((p) => p.user_id === userId);
        if (participant?.user_name) return participant.user_name;
        if (participant?.email) return participant.email;
        if (auth.user?.id === userId) return "나";
        return `사용자 ${userId}`;
    });
    ...
    const groupedFilteredOngoingNormalEvents = Object.values(
        filteredOngoingNormalEvents.reduce((acc, event) => {
            ...
            return acc;
        }, {} as Record<string, { date: Date; events: EventsData[] }>)
    ).sort((a, b) => a.date.getTime() - b.date.getTime());
    const koreanDate: string[] = ["일", "월", "화", "수", "목", "금", "토"];
    const renderNowHeader = () => (
        <div
            className="px-5 py-5 text-xs font-semibold flex justify-between items-center normal-text sticky top-0 z-[1] bg-white dark:bg-gray-950">
            ...
            <button onClick={() => {setFullSizeSideBarToggle(!fullSizeSideBarToggle)}} className="md:hidden block">
                <FontAwesomeIcon icon={!fullSizeSideBarToggle ? faUpRightAndDownLeftFromCenter : faDownLeftAndUpRightToCenter} />
            </button>
        </div>
    );
    ...
    const formatGroupDateLabel = (date: Date): string => {
        ...
        if (sameYear) {
            return `${String(date.getMonth() + 1).padStart(2, "0")} ${String(date.getDate()).padStart(2, "0")}(${koreanDate[date.getDay()]})`;
        }
    ...
    return (
        <div
            ref={sideBarRef}
            className={`z-[2] border-l overflow-y-auto overflow-x-hidden border-gray-300 dark:border-gray-800 duration-300 transition-[right,width] ${sideBar <= 0 ? (sideBarToggle ? `fixed top-0 md:top-[70px] h-[calc(100%-64px)] md:h-[calc(100vh-70px)] right-0 pointer-events-auto ${fullSizeSideBarToggle ? "w-full" : "w-[calc(100%-60px)]"}` : "-right-[100%] fixed top-0 md:top-[70px] h-[calc(100%-64px)] md:h-[calc(100vh-70px)] pointer-events-none w-[250px]") : "sticky top-0 w-[250px]"} max-h-full md:max-h-[calc(100vh-70px)] bg-white dark:bg-gray-950 normal-text user-select-none`}
        >
            {
                (contentMode === "normal") ? (
                    <div className="flex flex-col">
                        ...
                        {(eventId || (startAt && endAt)) ? (
                            <>
                                <div className="space-y-5">
                                    <EventTitleControl
                                        ...
                                        updateEvent={updateEvent} eventTitle={eventTitle}
                                        setEventTitle={setEventTitle}/>
                                    <EventDateViewAndControl
                                        disabled={(!!eventId && !(IsEditAuthority === "owner" || IsEditAuthority === "editor"))}
                                        contentMode={contentMode} startAt={startAt} setStartAt={setStartAt}
                                        endAt={endAt} setEndAt={setEndAt}/>
                                    <ParticipantControl participantControl={participantControl}
                                                        setParticipantControl={setParticipantControl}
                                                        setModalType={setModalType} setModalTitle={setModalTitle}
                                                        setModalMessage={setModalMessage} setModal={setModal}
                                                        eventUserControl={eventUserControl}
                                                        setEventUserControl={setEventUserControl}
                                                        onlineParticipantIds={onlineParticipantIds}
                                                        setEvents={setEvents} resetEvent={resetEvent}
                                                        IsEditAuthority={IsEditAuthority}
                                                        disabled={(!!eventId && !(IsEditAuthority === "owner"))}
                                                        saveEvent={saveEvent} eventId={eventId}
                                                        eventParticipants={eventParticipants}
                                                        setEventParticipants={setEventParticipants} auth={auth}/>
                                    {editingParticipantLabels.length > 0 ? (
                                        <div className="px-5">
                                            <div
                                                className="rounded border border-blue-200 dark:border-blue-800 bg-blue-50/80 dark:bg-blue-950/30 px-3 py-2">
                                                <p className="text-[11px] font-semibold text-blue-700 dark:text-blue-300">현재
                                                    편집중</p>
                                                <p className="text-[11px] text-blue-700 dark:text-blue-300 mt-0.5">{editingParticipantLabels.join(", ")}</p>
                                            </div>
                                        </div>
                                    ) : null}
                                    <EventDescriptionControl
                                        ...
                                        summarizeWithAi={summarizeEventWithAi}
                                        applySummaryToDescription={applyAiSummaryToDescription}
                                        onEditingStatusChange={setDescriptionEditingStatus}
                                    />
                                    <div className="px-5">
                                        <EventColorControl
                                            disabled={(!!eventId && !(IsEditAuthority === "owner" || IsEditAuthority === "editor"))}
                                    ...
                                    <ReminderControl eventReminder={eventReminder} addEventReminder={addEventReminder}
                                                     removeEventReminder={removeEventReminder}/>
                                </div>
                                <div
                                    className="bg-white dark:bg-gray-950 p-5 border-t border-gray-300 dark:border-gray-800 space-y-3">
                                    {
                                    ...
                                    />
                                    <div
                                        className="space-y-1 max-h-[240px] overflow-x-hidden overflow-y-auto hidden-scroll">
                                        {filteredOngoingNormalEvents.length <= 0 ? (
                                            <p className="text-xs text-gray-500 dark:text-gray-400 py-3 text-center">진행중인
                                                이벤트가 없습니다.</p>
                                        ) : (
                                            groupedFilteredOngoingNormalEvents.map((group) => (
                                                <div key={`normal-group-${group.date.getTime()}`}
                                                     className="space-y-1.5">
                                                    <p className="text-xs font-semibold normal-text">{formatGroupDateLabel(group.date)}</p>
                                                    {group.events.map((event) => (
                                                        ...
                                                        >
                                                            <div
                                                                className={`w-[4px] ${event.color} rounded shrink-0`}></div>
                            ...
                            <div
                                className="px-5 flex-1 pb-5 overflow-y-auto overflow-x-hidden relative flex flex-col space-y-5">
                                <ReminderView handleEventClick={handleEventClick} events={events} eventId={eventId}
                                              now={now} reminders={reminders}/>
                    ...
                    <>
                        ...
                        {activeChallenge ? (
                            ...
                            <>
                                ...
                                <div
                                    className="sticky bottom-0 bg-white dark:bg-gray-950 p-5 border-t border-gray-300 dark:border-gray-800">
                                    <button
                                        type="button"
