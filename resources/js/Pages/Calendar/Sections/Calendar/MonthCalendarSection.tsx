...
import {CalendarAtData, EventReminderItem, ParticipantsData} from "./CalendarData";
                    ...
                    {(() => {
                        ...
                        return (
                            weeks.map((week:CalendarAtData[], index:number) => {
                                ...
                                const usedRowCount = includeEvents.length > 0
                                    ? Math.max(...includeEvents.map((item) => item.row)) + 1
                                    : 0;
                                const shouldShowMoreButton = usedRowCount >= maxRows || rawIncludeEvents.length > includeEvents.length;
                                const bottomRow = Math.max(0, maxRows - 1);
                                const bottomRowEvents = includeEvents
                                    .filter((item) => item.row === bottomRow)
                                    .sort((a, b) => a.start_area - b.start_area);
                                const bottomAnchorEvent = bottomRowEvents[0] ?? null;
                                    ...
                                    <div key={index} className="grid grid-cols-7 text-right flex-1 snap-start week relative">
                                        ...
                                        {shouldShowMoreButton && (
                                            <div
                                                ...
                                                style={{
                                                    top: `${paddingVertical + availableHeight - 5}px`,
                                                    left: bottomAnchorEvent
                                                        ? `${(bottomAnchorEvent.start_area / 7) * 100}%`
                                                        : "0px",
                                                }}
                                            >
                                                <button
                                                    type="button"
                                                    onClick={() => {
                                                        const targetDate = bottomAnchorEvent
                                                            ? new Date(firstDate.getFullYear(), firstDate.getMonth(), firstDate.getDate() + bottomAnchorEvent.start_area)
                                                            : firstDate;

                                                        setActiveAt(new Date(targetDate.getFullYear(), targetDate.getMonth(), 1));
                                                        setActiveDay(targetDate.getDate());
                                                        setViewMode("week");
                                                    }}
                                                    className="text-[10px] md:text-xs underline text-gray-500 font-semibold ml-1 cursor-pointer"
                                                >
                                                    더보기
                                                </button>
                                            </div>
