...
export default function WriteSection({
    ...
    const normalizedLegacyRef = useRef<boolean>(false);
    ...
    const [mobileToolboxOpen, setMobileToolboxOpen] = useState<boolean>(false);
    ...
    const isLikelyLegacyHtml = (text: string): boolean => {
        return /<\/?(p|div|span|strong|b|em|i|u|a|ul|ol|li|h[1-6]|blockquote|pre|code|br)[^>]*>/i.test(text);
    };

    const legacyHtmlToMarkdown = (html: string): string => {
        ...
        const walk = (node: Node): string => {
            ...
            const trimChild = childrenText.trim();

            if (tag === "br") return "\n";
            if (tag === "strong" || tag === "b") return `**${trimChild}**`;
            if (tag === "em" || tag === "i") return `*${trimChild}*`;
            if (tag === "code" && el.parentElement?.tagName.toLowerCase() !== "pre") return `\`${trimChild}\``;
            if (tag === "pre") return `\n\`\`\`\n${trimChild}\n\`\`\`\n`;
            if (tag === "blockquote") return trimChild.split("\n").map((line) => `> ${line}`).join("\n");
            if (tag === "a") {
                const href = el.getAttribute("href") ?? "";
                return href ? `[${trimChild || href}](${href})` : trimChild;
            }
            if (/^h[1-6]$/.test(tag)) {
                const level = Number(tag[1]);
                return `${"#".repeat(level)} ${trimChild}\n`;
            }
            if (tag === "li") return trimChild;
            if (tag === "ul") {
            ...
            if (tag === "p" || tag === "div") return `${trimChild}\n`;
    ...
    useEffect(() => {
        if (normalizedLegacyRef.current) return;
        normalizedLegacyRef.current = true;

        const current = (notepadText ?? "").trim();
        if (!current || !isLikelyLegacyHtml(current)) return;

        const converted = legacyHtmlToMarkdown(current);
    ...
    }, [notepadText, handleSaveNotepadContent, setSaveStatus]);
    ...
    const markdownToHtml = (markdown: string): string => {
        ...
        for (const line of lines) {
            ...
            if (checkItem) {
                ...
                html.push(`<li><span>${checked ? "☑" : "☐"}</span> ${renderInline(checkItem[2] ?? "")}</li>`);
                continue;
            }
    ...
    const renderedPreview = useMemo(() => markdownToHtml(notepadText || ""), [notepadText]);
    const renderedAiSummary = useMemo(() => markdownToHtml(aiSummary ?? ""), [aiSummary]);

    const insertAtCursor = (prefix: string, suffix = "", defaultText = "") => {
        const textarea = textareaRef.current;
        if (!textarea) {
            setNotepadText((prev) => `${prev ?? ""}${prefix}${defaultText}${suffix}`);
            return;
        }
        ...
        const next = `${notepadText.slice(0, start)}${prefix}${inner}${suffix}${notepadText.slice(end)}`;
        setNotepadText(next);
    ...
    const applySummaryToNotepad = () => {
        const summary = (aiSummary ?? "").trim();
        if (!summary) return;
        setNotepadText((prev) => {
            const current = prev ?? "";
            if (!current.trim()) return summary;
            return `${current}\n\n${summary}`;
        });
    };
    ...
    const toolbarButtons = [
        { key: "h2", label: "제목", icon: faHeading, onClick: () => insertAtCursor("## ", "", "섹션 제목") },
        ...
        { key: "codeblock", label: "코드블록", icon: faCode, onClick: () => insertAtCursor("```txt\n", "\n```", "내용") },
        { key: "table", label: "표", icon: faTableCells, onClick: () => insertAtCursor("| 항목 | 내용 |\n| --- | --- |\n", "", "| 예시 | 값 |") },
    ...
    return (
        <div className="write-notepad flex flex-col flex-1 min-h-0 bg-gray-100 dark:bg-gray-950">
            <div className="md:hidden w-full box-border px-3 pt-3">
                <div className="grid grid-cols-2 gap-2">
                    <button
                        type="button"
                        onClick={() => setMobilePanel("write")}
                        className={`w-full rounded py-2 text-xs font-semibold transition-colors ${
                            mobilePanel === "write" ? "bg-blue-500 text-white" : "border border-gray-300 dark:border-gray-800 bg-white dark:bg-gray-900 normal-text"
                        }`}
                    >
                        작성
                    </button>
                    <button
                        ...
                        className={`w-full rounded py-2 text-xs font-semibold transition-colors ${
                            mobilePanel === "preview" ? "bg-blue-500 text-white" : "border border-gray-300 dark:border-gray-800 bg-white dark:bg-gray-900 normal-text"
                        }`}
                    >
                        미리보기
                    </button>
            ...
            <div className={`px-3 md:px-4 pt-3 pb-2 border-b border-gray-300 dark:border-gray-800 space-y-2 bg-white dark:bg-gray-950 shrink-0 ${mobilePanel === "preview" ? "hidden md:block" : ""}`}>
                <div className="rounded border border-gray-300 dark:border-gray-800 bg-gray-50 dark:bg-gray-900/40 p-2 space-y-1.5">
                    <button
                        type="button"
                    ...
                    >
                        <div>
                            ...
                            <p className="text-[10px] text-gray-500 dark:text-gray-400">메모와 별도로 저장됩니다</p>
                        </div>
                        <span className="inline-flex items-center gap-1 text-[10px] normal-text border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-900 rounded px-2 py-1 shrink-0">
                            {aiPanelOpen ? "접기" : "열기"}
                            <FontAwesomeIcon icon={aiPanelOpen ? faChevronUp : faChevronDown} className="text-[10px]" />
                        </span>
                    </button>
                    {aiPanelOpen ? (
                        <>
                            <textarea
                                className="w-full min-h-[56px] max-h-[110px] border border-gray-300 dark:border-gray-700 rounded bg-gray-50 dark:bg-gray-900 text-xs p-2 outline-none resize-y normal-text"
                            ...
                            <div className="flex flex-wrap gap-1.5">
                                ...
                                <button
                                    type="button"
                                    disabled={(aiSummary ?? "").trim().length <= 0}
                                    onClick={applySummaryToNotepad}
                ...
                <div className="md:hidden">
                    <div
                        ...
                        className="w-full inline-flex items-center justify-between px-2.5 py-2 rounded border border-gray-300 dark:border-gray-800 normal-text bg-gray-50 dark:bg-gray-900/40"
                    >
                        <div className="text-left">
                            <p className="text-[11px] font-semibold normal-text">작성 도구</p>
                            <p className="text-[10px] text-gray-500 dark:text-gray-400">다양한 스타일로 메모하세요</p>
                        </div>
                        <button className="inline-flex items-center gap-1 text-[10px] normal-text border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-900 rounded px-2 py-1 shrink-0">
                            {mobileToolboxOpen ? "접기" : "열기"}
                            <FontAwesomeIcon icon={mobileToolboxOpen ? faChevronUp : faChevronDown} className="text-[10px]" />
                        </button>
                    </div>
                </div>
                <div className={`space-y-2 ${mobileToolboxOpen ? "block" : "hidden"} md:block`}>
                <div className="flex flex-wrap md:flex-nowrap gap-1.5 md:overflow-x-auto overflow-x-visible hidden-scroll whitespace-normal md:whitespace-nowrap pb-1 min-w-0">
                    {toolbarButtons.map((tool) => (
                        <button
                            key={tool.key}
                            type="button"
                            onClick={tool.onClick}
                            className="inline-flex items-center gap-1 px-2 py-1 rounded border border-gray-300 dark:border-gray-700 text-[11px] normal-text hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors md:shrink-0"
                            title={tool.label}
                        >
                ...
                <div className="flex items-center flex-wrap md:flex-nowrap gap-1.5 md:overflow-x-auto overflow-x-visible hidden-scroll whitespace-normal md:whitespace-nowrap pb-1 min-w-0">
                    <button
                        type="button"
                        onClick={() => setColorTarget("text")}
                            className={`px-2 py-1 rounded border text-[11px] font-semibold transition-colors md:shrink-0 ${
                                colorTarget === "text" ? "bg-blue-500 text-white border-blue-500" : "border-gray-300 dark:border-gray-700 normal-text"
                            }`}
                    >
                        텍스트색
                    </button>
                    <button
                        ...
                        onClick={() => setColorTarget("bg")}
                            className={`px-2 py-1 rounded border text-[11px] font-semibold transition-colors md:shrink-0 ${
                                colorTarget === "bg" ? "bg-blue-500 text-white border-blue-500" : "border-gray-300 dark:border-gray-700 normal-text"
                            }`}
                    >
                        배경색
                    </button>
                    {colorPresets.map((color) => (
                        <button
                            ...
                            className={`inline-flex items-center px-2 py-1 rounded border text-[11px] font-semibold transition-colors normal-text md:shrink-0 ${color.className}`}
                        >
                            {color.label}
                        </button>
                    ))}
                    <input
                        ...
                        className="size-7 rounded border border-gray-300 dark:border-gray-700 bg-transparent cursor-pointer md:shrink-0"
                        aria-label="커스텀 색상"
                    />
                    <button
                        type="button"
                        ...
                        className="inline-flex items-center px-2 py-1 rounded border border-gray-300 dark:border-gray-700 text-[11px] font-semibold normal-text hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors md:shrink-0"
                    >
                        적용
                    </button>
                </div>
                <div className="flex flex-wrap md:flex-nowrap gap-1.5 md:overflow-x-auto overflow-x-visible hidden-scroll whitespace-normal md:whitespace-nowrap pb-1 min-w-0">
                    <button
                        ...
                        className="inline-flex items-center px-2 py-1 rounded border border-gray-300 dark:border-gray-700 text-[11px] normal-text hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors md:shrink-0"
                    >
                        공개 템플릿 고르기
                    </button>
                    <button
                    ...
                    <button
                        ...
                        className="inline-flex items-center px-2 py-1 rounded border border-blue-500 text-[11px] text-blue-600 dark:text-blue-300 hover:bg-blue-50 dark:hover:bg-blue-950/30 transition-colors md:shrink-0"
                    >
                        템플릿 만들기
                    </button>
                </div>
            ...
            <div className="flex-1 min-h-0 grid grid-cols-1 md:grid-cols-2 overflow-hidden">
                <div className={`min-h-[360px] md:min-h-0 p-3 md:p-4 border-b md:border-b-0 md:border-r border-gray-300 dark:border-gray-800 bg-white dark:bg-gray-950 ${mobilePanel === "preview" ? "hidden md:block" : ""}`}>
                    <textarea
                        ref={textareaRef}
                        className="w-full h-full border border-gray-300 rounded p-3 text-xs font-semibold text-gray-900 bg-white outline-none resize-none"
                        placeholder="메모를 Markdown으로 작성하세요..."
                        value={notepadText}
                        onChange={(e) => setNotepadText(e.target.value)}
                    />
                </div>
                <div className={`min-h-[360px] md:min-h-0 p-3 md:p-4 overflow-auto bg-gray-50/70 dark:bg-gray-900/30 ${mobilePanel === "write" ? "hidden md:block" : ""}`}>
                    {notepadText.trim().length > 0 ? (
                        ...
                        <p className="text-xs text-gray-500 dark:text-gray-400">내용을 입력하면 미리보기가 표시됩니다.</p>
                    )}
                </div>
            </div>
