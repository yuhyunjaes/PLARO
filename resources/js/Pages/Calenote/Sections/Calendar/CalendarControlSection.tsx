...
import {faAngleDown, faAngleLeft, faAngleRight, faAngleUp, faCalendarDays, faChevronDown} from "@fortawesome/free-solid-svg-icons";
...
export default function CalendarControlSection({ getWeekWednesday, activeAtToToday, contentMode, setContentMode, setFirstCenter, setMonths, setTemporaryYear, setTemporaryMonth, setTemporaryDay, setIsDragging, startAt, viewMode, setViewMode, activeAt, setActiveAt, activeDay, setActiveDay}: CalendarControlSectionProps) {
    ...
    }, [contentChangeToggle, dateJumpToggle]);
        ...
        <div className="rounded-xl flex justify-between items-center px-5 pt-5">
            <div className="flex items-center space-x-1 relative">
                <div ref={dateJumpAreaRef} className="relative">
                    <button
                        type="button"
                        className="flex items-center gap-1 text-lg md:text-2xl normal-text font-semibold user-select-none cursor-pointer hover:text-blue-600 dark:hover:text-blue-300 transition-colors"
                    ...
                    >
                        <span>
                            ...
                            {viewMode !== "month" && (
                                <>-{(baseDayForSingleView > 9) ? baseDayForSingleView : `0${baseDayForSingleView}`}일</>
                            )}
                        </span>
                        <FontAwesomeIcon icon={faCalendarDays} className="text-xs md:text-sm" />
                    </button>
            ...
            <div className="flex items-center relative">
                <div ref={contentChangeAreaRef} className="relative mr-1">
                    <button
                        ...
                        className={`rounded border px-2 md:px-3 py-1 text-[10px] md:text-xs font-semibold transition-colors cursor-pointer ${contentModeMeta[contentMode].activeClass}`}
                        aria-live="polite"
                    ...
                    {contentChangeToggle ? (
                        <div className="flex flex-col min-w-[90px] bg-white dark:bg-gray-950 border border-gray-300 dark:border-gray-800 rounded-md p-1 space-y-1 user-select-none absolute top-full mt-1.5 z-[20] left-0 shadow-md">
                            <button
                                type="button"
                                onClick={() => setContentMode("normal")}
                                className={`w-full text-left px-2 py-1.5 text-xs font-semibold rounded transition-colors duration-150 cursor-pointer ${
                                    contentMode === "normal"
                                        ? "bg-blue-500 text-white"
